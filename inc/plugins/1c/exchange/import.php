...
function wc1c_replace_product($is_full, $guid, $product) {
  ...
  if ($product['ЗначенияСвойств']) {
    ...
    foreach ($product['ЗначенияСвойств'] as $property) {
      ...
      if ($attribute_terms || $attribute_values) {
        ...
        // test00 КАЖДЫЙ атрибут, убрать метка, топ продаж
        ...
        if($product_attribute['name'] === 'Высота'){
          $p_sim = wc_get_product( $post_id );
          $p_sim->set_height(isset( $product_attribute['value'] ) ? $product_attribute['value']: '');
          $p_sim->save();
          continue;
        }    
        if($product_attribute['name'] === 'Ширина'){
          $p_sim = wc_get_product( $post_id );
          $p_sim->set_width(isset( $product_attribute['value'] ) ? $product_attribute['value']: '');
          $p_sim->save();
          continue;
        }    
        if($product_attribute['name'] === 'Длина'){
          $p_sim = wc_get_product( $post_id );
          $p_sim->set_length(isset( $product_attribute['value'] ) ? $product_attribute['value']: '');
          $p_sim->save();
          continue;
        }
    ...
    foreach ($terms as $attribute_taxonomy => $attribute_terms) {
      // test00 создать термины такономии для аттрибутов
  ...
  foreach ($product['ЗначенияРеквизитов'] as $requisite) {
    ...
    if(in_array($product_attribute_name, ['Путь к картинке', 'Товар отсутствует', 'Старый код']))continue;




     // test00 убрать странные вид тип путь товар
    $product_attributes[$product_attribute_key] = array(
      'name' => wc_clean($product_attribute_name),
